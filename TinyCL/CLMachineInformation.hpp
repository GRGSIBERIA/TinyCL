#ifndef CL_MACHINE_INFORMATION_HPP
#define CL_MACHINE_INFORMATION_HPP
...
namespace tcl
{
	...
	public:
		CLMachineInformation()
			:
			platformId(NULL), context(NULL), deviceInfos()
		{
			SetupPlatformIds();	// コンピュータのIDを取得

			SetupDeviceIds();		// 演算装置のIDを取得

			SetupDevicesInformations();	// デバイス情報を取得

			CreateContext();	// 中央管理的なもの
		}

		virtual ~CLMachineInformation()
		{
